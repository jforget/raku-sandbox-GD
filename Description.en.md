...
Version 0.7  aims at  using styles  when drawing  lines and  the like.
Before modifying the Raku module, I have a few experiments to run.

### Storing a style into file `foo.gd`

The idea is to check whether a  first program can create an image with
a style and  store this image into  a file `foo.gd` et  then whether a
