...
image. Maybe some other distributions do things differently.
...
The module is installed with just `zef install GD::Raw`, because by default
`libgd3` is already installed on  this distribution. As I have already
written when listing my needs, several features are missing:
...
and pseudo colour `gdStyled` allowing  to draw dotted lines and dashed
...
and heights.  Glyphs are defined  only for the space and the 7 letters
...
In the  same way, error processing  is important. There is  a definite
possibility that the user might include a typo in the font name or the
font directory.  In this case, the  Raku module must send  to the user
the message provided by `libgd`.
...
see  that the  files  are very  similar.  At first,  I  used the  file
generated by the Raku test script as the expected file. Then I noticed
that anti-aliasing  can be disabled  by using  a colour number  with a
"minus" sign.  I disabled  anti-aliasing in both  `mk-test-ttf.pl` and
`xt/gdimagestringft.rakutest`  and now  it  works with  the test  file
generated by `mk-test-ttf.pl`.

These precautions were not sufficient to validate the
[pull request](https://github.com/raku-community-modules/GD-Raw/pull/6)
I submitted on June 4th.  Because of tiny differences when pinpointing
the chars,  some pixels  are white in  a file and  black in  the other
file.  I fixed  the  test script  `gdimagestringft.rakutest` to  allow
...
To be frank, I am not sure  I understand why these functions should be
used. As far as
[`gdImageDestroy`](https://libgd.github.io/manuals/2.3.3/files/gd-c.html#gdImageDestroy)
and [`gdFree`](https://libgd.github.io/manuals/2.3.3/files/gdhelpers-c.html)
are concerned, they are used to deallocate a buffer which is obviously
no  longer useful.  Here, these  functions deal  with a  cache storing
fonts that have  already been used and  that may be used  again in the
future.  Yet,  I consider  that  these  functions  must be  ported  to
`GD::Raw` if `gdImageStringFT` has been ported.
